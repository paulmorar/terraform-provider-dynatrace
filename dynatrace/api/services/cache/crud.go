...
type crudService[T api.Settings] struct {
	service api.CRUDService[T]
	folder  string
	index   *stubIndex
}
...
func (me *crudService[T]) Validate(v T) error {
	...
	if validator, ok := me.service.(api.Validator[T]); ok {
...
func CRUD[T api.Settings](service api.CRUDService[T]) api.CRUDService[T] {
