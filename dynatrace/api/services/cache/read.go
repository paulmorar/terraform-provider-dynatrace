...
type readService[T api.Settings] struct {
	service api.RService[T]
	folder  string
	index   *stubIndex
}
...
func (me *readService[T]) list(withValues bool) (api.Stubs, error) {
	...
	if exists(indexFile(me.folder)) {
		...
		if withValues && index.Complete {
			for _, stub := range index.Stubs {
				stub.Value = api.NewSettings[T](me)
	...
	if me.index != nil {
		...
		for _, entry := range entries {
			...
			if record.Value != nil {
				id := record.ID
				settings = api.NewSettings(me.service)
				if err = api.FromJSON(record.Value, settings); err != nil {
					return nil, err
				}
				if legacyIDAware, ok := me.service.(v2.LegacyIDAware); ok {
					api.SetLegacyID(id, legacyIDAware.LegacyID(), settings)
				}
			}
			stubs = append(stubs, &api.Stub{ID: record.ID, Name: record.Name, Value: settings})
		}
...
func (me *readService[T]) Get(id string, v T) error {
	...
	if cache, err = me.loadConfig(id, v); err != nil {
		...
		if legacyIDAware, ok := me.service.(v2.LegacyIDAware); ok {
			api.SetLegacyID(id, legacyIDAware.LegacyID(), v)
		}
...
func (me *readService[T]) loadIndex() (*stubIndex, error) {
	...
	me.index.IDs = map[string]*api.Stub{}
...
func (me *readService[T]) loadConfig(id string, v T) (bool, error) {
	...
	if exists(filePath) {
		...
		if err = api.FromJSON(record.Value, v); err != nil {
			return false, err
		}
		if legacyIDAware, ok := me.service.(v2.LegacyIDAware); ok {
			api.SetLegacyID(id, legacyIDAware.LegacyID(), v)
...
func Read[T api.Settings](service api.RService[T]) api.RService[T] {
	...
	if ncs, ok := service.(api.NoCacheService); ok && ncs.NoCache() {
		return service
	}
